...
A_values = np.linspace(0.0001, 0.08, 75)  # Values of A
alpha_values = np.linspace(0.5, 5, 75)  # Values of omega
...
for i, alpha in enumerate(alpha_values):
    for j, A in enumerate(A_values):
        ...
        Pe_eta_values[i, j] = alpha**4 * A**2 * S + A * alpha**2 * S * eps
...
Pe_min = As[0]**2 * alpha_values[0]**4 * S

A_balance = eps / (alpha_values**2)
plt.plot(A_balance, alpha_values, color='white', linestyle='--', linewidth=1.5, label=r'$Pe_{osc} = Pe_{pd}$')

A_big = 1 / (alpha_values**2)
plt.plot(A_big, alpha_values, color='white', linestyle='-.', linewidth=1.5, label=r'$Pe_{osc} = Pe_{pd}/\epsilon$')
...
#contour = plt.contourf(A_grid, omega_grid, Pe_eta_values,
                    #    levels=100,
                    #    cmap=custom_plasma())
cbar = plt.colorbar(contour, label=r'$Pe_{total}$')
...
markers = [
    ...
    {'alpha': 2.5, 'A': 0.0025*10},
    {'alpha': 2.5, 'A': 0.0025*10*np.sqrt(10)}
]
print(markers)
...
plt.plot(As, alpha_values, color='white')
...
plt.savefig('outputs/pe_total_regime_logscale.png')