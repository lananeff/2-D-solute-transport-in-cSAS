...
plt.plot(As, max_u1, label='$ u_s$', color='blue')
#plt.plot(As, max_us, label=r'$u_{\omega}$', linestyle='--', color='blue')

plt.plot(As, max_ul, label='$u_L$', linestyle = ':', color='blue', linewidth =4)

plt.plot(As, max_upd, label='$u_{pd}$', color = 'blue', linestyle = 'dashdot')
...
plt.xlabel(r'$A$')
plt.ylabel('Max Value (m/s)')
plt.yscale('log')
plt.xscale('log')
plt.legend()

# Save the plot to the 'outputs' folder
plt.savefig('outputs/dimensional_comparison_u1_vs_us_varying_A.png')
