...
gs = plt.GridSpec(3, 7, width_ratios=[1, 1, 0.05, 1, 0.05, 1, 0.05], wspace=0.3)

axes = []
for i in range(3):         # rows
    for j in [0, 1, 3, 5]: # skip 2 (first colorbar) and 5 (second colorbar)
...
for k in range(3):
    ...
    fig.text(0.725, y_position, panel_labels[2*k+1], fontsize=12, ha='right', va='center', fontweight='bold')
...
for row in range(3):
    ...
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    sm = ScalarMappable(norm=norms[row * 3 + 2], cmap=cmaps[row])
    sm.set_array([])
    cbar = fig.colorbar(sm, cax=cax, orientation='vertical')
