...
if __name__ == "__main__":
    ...
    parser.add_argument("--species", choices=["human", "mouse"], default ="human", help="Choose species: human or mouse. Default reproduces Fig 4.")
    parser.add_argument("--regime", choices=["cardiac", "respiratory", "sleep"], default=None, help="Choose flow regime. Default reproduces Fig 4.")
    parser.add_argument("--velocity_type", choices=["leading_order", "steady_streaming", "stokes_drift", "lagrangian_velocity", "pd", "lag_pd"], 
                            default="steady_streaming,stokes_drift,pd", 
                            help="Select velocity type. For multiple use comma-separated velocity types. Default reproduces Fig 4.")
    ...
    if len(velocity_types) == 1:
        ...
        plot_velocity_profile(velocity_types[0], args.species, args.regime, alpha, A, eps, omega, h, S)
    else:
        # Call the multiple velocity types plot function
        plot_multiple_velocity_profiles(velocity_types, args.species, args.regime, alpha, A, eps, omega, h, S)
