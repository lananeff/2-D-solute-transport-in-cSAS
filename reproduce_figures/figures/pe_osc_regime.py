...
A_values = np.linspace(0.0005, 0.5, 200)  # Values of A
alpha_values = np.linspace(0.5, 7, 125)  # Values of omega
...
for i, alpha in enumerate(alpha_values):
    for j, A in enumerate(A_values):
        ...
        #Pe_eta_values[i, j] = alpha**2 * A * S * max_vL_values[i, j]
        Pe_eta_values[i, j] = alpha**4 * A**2 * S * vscale


vscale=0.002
pdscale = 1.27 #with S=4096
...
Atds = 1 / (alpha_values**2 * S )
# Find the minimum Pe_eta along this line
Pe_min = As[0] * alpha_values[0]**2 * S *vscale

A_balance = np.sqrt(eps * pdscale / ( alpha_values**4 * S * vscale)) # 0.00055 comes from vlmax as varied with alpha A=0.0025
plt.plot(A_balance, alpha_values, color='white', linestyle='--', linewidth=1.5, label=r'$Pe_{osc} = \epsilon Pe_{pd}$')

A_big = np.sqrt(pdscale / (alpha_values**4 * S * vscale))
plt.plot(A_big, alpha_values, color='white', linestyle='-.', linewidth=1.5, label=r'$Pe_{osc} = Pe_{pd}$')
...
#contour = plt.contourf(A_grid, omega_grid, Pe_eta_values,
                    #    levels=100,
                    #    cmap=custom_plasma())
cbar = plt.colorbar(contour, label=r'$Pe_{osc}$')
...
markers = [
    {'alpha': 2.5, 'A': 0.0025/np.sqrt(10)},
    {'alpha': 2.5, 'A': 0.0025*np.sqrt(10)},
    {'alpha': 2.5, 'A': 0.0025*np.sqrt(10)**3}
]
print(markers)
...
for marker in markers:
    ...
    i_marker = np.abs(alpha_values - omega_marker).argmin()
    j_marker = np.abs(A_values - A_marker).argmin()
    Pe_eff_marker = Pe_eta_values[i_marker, j_marker]
    ...
    if regime_name:
        label = label = rf"{regime_name}: $Pe_{{osc}} \approx {Pe_eff_rounded}$"
    else:
        label = label = rf"$Pe_{{osc}} \approx {Pe_eff_rounded}$" # No regime, just Pe value
...
#plt.plot(Atds, alpha_values, color='white')
...
plt.xlabel('A')
plt.ylabel(r'$\alpha$')
#plt.title(r'Effective Peclet Number $Pe_{osc}$')
...
plt.savefig('outputs/pe_osc_regime_logscale.png')