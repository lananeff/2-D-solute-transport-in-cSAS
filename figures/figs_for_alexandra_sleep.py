...
xn = 50
yn = 50
As = np.linspace(20*5e-5, 30*1e-2, 75)
...
ax_summary.plot(As*h, max_u1, label=r'$ \tilde{u}_s$', color='blue', linewidth=1)
ax_summary.plot(As*h, max_us, label=r'$\tilde{u}_{\omega}$', linestyle='--', color='blue', linewidth=1)
ax_summary.plot(As*h, max_ul, label=r'$\tilde{u}_L$', linestyle=':', color='blue', linewidth=2)
ax_summary.plot(As*h, max_upd, label=r'$\tilde{u}_{pd}$', color='blue', linestyle='dashdot', linewidth=1)
A_vals = [(20*1e-4 )*h, (20*5e-3*h), (20*1e-2)*h]  # Low, mid, high
labels = ['(i)', '(ii)', '(iii)']

x_center = 6.6e-6 * 20
x_err = 2.9e-6 * 20
x_min, x_max = (x_center - x_err), (x_center + x_err)

ax_summary.axvspan(x_min, x_max, color="grey", alpha=0.3)
...
A_vals = [20*1e-4, 20*5e-3, 20*1e-2]  # Low, mid, high
...
plt.savefig('outputs/max_vs_A_and_stacked_profiles_sleep.png', bbox_inches='tight')
