...
image_folder = "./figures/paraview figs/fig 10/ss/pd/combos/"
image_files = [os.path.join(image_folder, f"combined_images_{i+1}.png") for i in range(5)]
...
titles = [r"$Pe_{osc} = 0.01$", r"$Pe_{osc} = 0.1$", r"$Pe_{osc} = 1$", r"$Pe_{osc} =10$", r"$Pe_{osc} = 100$"]
...
for i, img_file in enumerate(image_files):
    if os.path.exists(img_file):
        ...
        ax.set_yticks([height * 0, height * 0.4725, height * 0.9125])
...
plt.tight_layout()
plt.savefig('outputs/spine_leak_pd_ss.png')
